...
// bass leads: soft-clipped saw riff + 2Hz tremolo = sidechain pump feel
parts.push(`[4:a]asoftclip=type=atan,tremolo=f=2:d=0.45,volume=0.62,adelay=${DROP_MS}|${DROP_MS}[bass]`);
labels.push('bass');
// four-on-the-floor kick (0.5s loop = 120bpm quarters)
parts.push(`[1:a]volume=0.55,adelay=${DROP_MS}|${DROP_MS}[kick]`); labels.push('kick');
// constant 16th hats, brighter than v1
parts.push(`[2:a]volume=0.22,adelay=${DROP_MS}|${DROP_MS}[hat]`); labels.push('hat');
// claps on the backbeat (1.0s loop, hit at 0.5s)
parts.push(`[5:a]volume=0.4,adelay=${DROP_MS}|${DROP_MS}[clap]`); labels.push('clap');
